...
function fxDTSBrick::setVehicle(%obj, %data, %client)
{
	...
	if (%data.getClassName() $= "PlayerData")
	{
		...
		if (!$Server::LAN)
		{
			if (%brickGroup.numPlayerVehicles >= $Server::Quota::Player)
			{
				if (%client.brickGroup == %brickGroup)
				{
					...
					{
						commandToClient(%client, 'CenterPrint', "\c0You already have " @ $Server::Quota::Player @ " player-vehicles", 2);
					}
				}
				...
				{
					commandToClient(%client, 'CenterPrint', "\c0" @ %brickGroup.name @ " already has " @ $Server::Quota::Player @ " player-vehicles", 2);
				}
				return;
			}
		}
		if ($Server::numPlayerVehicles >= $Server::MaxPlayerVehicles_Total)
		{
			...
			{
				commandToClient(%client, 'CenterPrint', "\c0Server is limited to " @ $Server::MaxPlayerVehicles_Total @ " player-vehicles", 2);
			}
			return;
		}
	}
	else
	{
		...
		if (!$Server::LAN)
		{
			if (%brickGroup.numPhysVehicles >= $Server::Quota::Vehicle)
			{
				if (%client.brickGroup == %brickGroup)
				{
					...
					{
						commandToClient(%client, 'CenterPrint', "\c0You already have " @ $Server::Quota::Vehicle @ " physics-vehicles", 2);
					}
				}
				...
				{
					commandToClient(%client, 'CenterPrint', "\c0" @ %brickGroup.name @ " already has " @ $Server::Quota::Vehicle @ " physics-vehicles", 2);
				}
				return;
			}
		}
		...
		if ($Server::numPhysVehicles >= $Server::MaxPhysVehicles_Total)
		{
			...
			{
				commandToClient(%client, 'CenterPrint', "\c0Server is limited to " @ $Server::MaxPhysVehicles_Total @ " physics-vehicles", 2);
			}
			return;
		}
	}
